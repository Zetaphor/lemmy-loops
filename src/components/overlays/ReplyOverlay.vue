<template>
  <div class="fixed top-0 left-0 z-20 w-full h-full bg-gray-800">
    <div v-if="replyOverlay.isPost" class="w-full h-4/6 overflow-scroll">
      <p class="text-lg text-white p-2">{{ replyOverlay.data.content.name }}</p>

      <div class="pl-1">
        <div class="text-sm inline-flex items-center">
          <img v-if="replyOverlay.data.community.icon.length && preferences.showPostCommunityIcon"
            :src="replyOverlay.data.community.icon" class="h-4 w-4 rounded-sm mt-0.5 mr-1">
          <p class="text-sm flex-grow-0 text-gray-300">{{ replyOverlay.data.community.name }}</p>
          ...
          <p class="text-sm flex-grow-0 text-gray-400">{{ replyOverlay.data.community.actor_domain }}</p>
        </div>
      </div>
      <!-- Post creator -->
      <div class="pl-1">
        <div class="inline-flex items-center">
          ...
          <div class="avatar mr-0.5"
            v-if="replyOverlay.data.creator.avatar.length && preferences.showPostAvatars && preferences.showPostCustomAvatars">
            <div class="w-4 rounded-full mr-0.5">
              <img :src="replyOverlay.data.creator.avatar"
                :alt="`${replyOverlay.data.creator.name}@${replyOverlay.data.creator.actor_domain}`"
                :title="`${replyOverlay.data.creator.name}@${replyOverlay.data.creator.actor_domain}`" />
            </div>
          </div>
          ...
          <div v-else-if="preferences.showPostAvatars" class="mr-0.5">
            <Avatar :size="16" variant="beam"
              :name="`${replyOverlay.data.creator.name}@${replyOverlay.data.creator.actor_domain}`" />
          </div>
          ...
          <p class="text-sm flex-grow-0 ml-0.5">
            <span class="text-gray-300 text-md">{{ replyOverlay.data.creator.name }}</span>
            <span class="text-gray-400 text-md">@{{ replyOverlay.data.creator.actor_domain }}</span>
          </p>
        </div>
      </div>
      ...
      <p class="mt-1 text-md text-gray-200 bg-gray-600 rounded-md my-1 px-2 w-15 pl-2 pr-2 pt-2 pb-2">
        <Markdown v-if="!showPreview && replyOverlay.data.content.body.length" :source="replyOverlay.data.content.body" />
        <Markdown v-else-if="showPreview || !replyOverlay.data.content.body.length" :source="reply" />
      </p>
    </div>
    <div class="w-full h-2/6">
      ...
      <button class="btn bg-gray-500 hover:bg-gray-700">Submit</button>
...
onMounted(() => {
  if (replyOverlay.data.content.body.length) {
