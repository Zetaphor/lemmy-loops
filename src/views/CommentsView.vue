<template>
  <div class="w-screen h-screen relative overflow-scroll pt-2 pb-96">
    <CommentItem :comment="comment" :depth="1" v-for="(comment, index) in postComments" :key="index" />
...
async function updateComments() {
  comments.renderedNodes = 1
  postComments.value = []
  const commentData = await comments.getComments(props.post_id)
